#getting all records for all scraped place names
setwd("~/ownCloud/iaclals-scraping")
biorecord <- read.csv("biorecord.csv", stringsAsFactors = FALSE, header = FALSE)
terra.birth <- which(biorecord$V1=="Birth Place")
terra.death <- which(biorecord$V1=="Death Place")
terra.rows <- c(terra.birth,terra.death)
terra.places <- unique(biorecord$V2[terra.rows])
...
allplacedata <- data.frame(terra.places, areaTotal, populationTotal, populationTotalRanking, PopulatedPlace, type, hypernym, longd,
                           latd, long, lat, point, country, utcOffset)
write.csv(allplacedata, file = "allplacedata.csv", sep = ",", append = FALSE, 
