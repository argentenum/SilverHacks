setwd("~/Documents/fb-scraping")
directory <- "~/Documents/fb-scraping/saa-talks.back.txt"
...
library(igraph)
dat <- data.frame(tag.rel$post.by,tag.rel$tag.friend, row.names = NULL)
...
write.csv(tag.rel,paste(substring(basename(directory),1,(nchar(basename(directory))-4)),"-tag.history.csv",sep = ""),row.names = FALSE)
